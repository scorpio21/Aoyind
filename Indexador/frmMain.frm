...
Begin VB.Form frmMain 
   ...
   Begin VB.Menu FileMnu 
      ...
      End
      Begin VB.Menu mmCargarTXT 
         Caption         =   "Cargar desde TXT"
      End
      Begin VB.Menu mmGuardarTXT 
...
Sub DrawFrame(FrameIndex As Integer)
            ...
            If Right$(Config.bmpPath, 1) <> "\" Then
                path = Config.bmpPath & "\" & GrhData(GrhData(currentGrh).Frames(FrameIndex)).FileNum & ".png"
            Else
                path = Config.bmpPath & GrhData(GrhData(currentGrh).Frames(FrameIndex)).FileNum & ".png"
            End If
            ...
            If Dir(path) <> "" Then
            Set currentPic.Picture = LoadPictureGDI(path)
            End If
...
If Not FinCarga Then Exit Sub
    ...
    If Right$(Config.bmpPath, 1) <> "\" Then
        path = Config.bmpPath & "\" & bmpTxt.text & ".png"
    Else
        path = Config.bmpPath & bmpTxt.text & ".png"
    End If
...
For Each X In f.Files
    ...
    If Num > 0 Then
        If BMPenGRH(Num) = 0 Then
            FSO.MoveFile f.path & "\" & Num & ".png", App.path & "\NoUsados\" & Num & ".png"
        End If
    End If
Next X
End Sub
...
If BorrarBMP Then
    If Dir(Config.bmpPath & "\" & bmp & ".png") <> "" Then
        Kill (Config.bmpPath & "\" & bmp & ".png")
    End If
End If
        With GrhData(GH)
            .FileNum = 0
...
For Each X In f.Files
    ...
    If Num > 0 Then
        G = BMPenGRH(Num)
        If G = 0 Then
            FSO.CopyFile f.path & "\" & Num & ".png", App.path & "\Categorias\NoUsados\" & Num & ".png"
        Else
            Encontro = False
            For e = 0 To UBound(BodyData)
                If BodyData(e).Walk(1).grhindex = G Or BodyData(e).Walk(2).grhindex = G Or BodyData(e).Walk(3).grhindex = G Or BodyData(e).Walk(4).grhindex = G Then
                    FSO.CopyFile f.path & "\" & Num & ".png", App.path & "\Categorias\Cuerpos\" & Num & ".png"
                    Encontro = True
                    Exit For
                End If
            Next e
            For e = 0 To UBound(HeadData)
                If HeadData(e).Head(1).grhindex = G Or HeadData(e).Head(2).grhindex = G Or HeadData(e).Head(3).grhindex = G Or HeadData(e).Head(4).grhindex = G Then
                    FSO.CopyFile f.path & "\" & Num & ".png", App.path & "\Categorias\Cabezas\" & Num & ".png"
                    Encontro = True
                    Exit For
                End If
            Next e
            For e = 0 To UBound(CascoAnimData)
                If CascoAnimData(e).Head(1).grhindex = G Or CascoAnimData(e).Head(2).grhindex = G Or CascoAnimData(e).Head(3).grhindex = G Or CascoAnimData(e).Head(4).grhindex = G Then
                    FSO.CopyFile f.path & "\" & Num & ".png", App.path & "\Categorias\Cascos\" & Num & ".png"
                    Encontro = True
                    Exit For
                End If
            Next e
            For e = 0 To UBound(WeaponAnimData)
                If WeaponAnimData(e).WeaponWalk(1).grhindex = G Or WeaponAnimData(e).WeaponWalk(2).grhindex = G Or WeaponAnimData(e).WeaponWalk(3).grhindex = G Or WeaponAnimData(e).WeaponWalk(4).grhindex = G Then
                    FSO.CopyFile f.path & "\" & Num & ".png", App.path & "\Categorias\Armas\" & Num & ".png"
                    Encontro = True
                    Exit For
                End If
            Next e
            For e = 0 To UBound(ShieldAnimData)
                If ShieldAnimData(e).ShieldWalk(1).grhindex = G Or ShieldAnimData(e).ShieldWalk(2).grhindex = G Or ShieldAnimData(e).ShieldWalk(3).grhindex = G Or ShieldAnimData(e).ShieldWalk(4).grhindex = G Then
                    FSO.CopyFile f.path & "\" & Num & ".png", App.path & "\Categorias\Escudos\" & Num & ".png"
                    Encontro = True
                    Exit For
                End If
            Next e
            For e = 0 To UBound(FxData)
                If FxData(e).Animacion = G Then
                    FSO.CopyFile f.path & "\" & Num & ".png", App.path & "\Categorias\FX\" & Num & ".png"
                    Encontro = True
                    Exit For
                End If
            Next e
            For e = 0 To UBound(ObjData)
                If ObjData(e).grhindex = G Then
                    FSO.CopyFile f.path & "\" & Num & ".png", App.path & "\Categorias\Obj\" & Num & ".png"
                    Encontro = True
                    Exit For
                End If
            Next e
            ...
            If Not Encontro Then
                FSO.CopyFile f.path & "\" & Num & ".png", App.path & "\Categorias\Otros\" & Num & ".png"
            End If
        End If
    End If
Next X
...
FinCarga = False
    ...
    If GrhData(currentGrh).NumFrames > 0 Then
    If Right$(Config.bmpPath, 1) <> "\" Then
        path = Config.bmpPath & "\" & GrhData(GrhData(currentGrh).Frames(CurrentFrame)).FileNum & ".png"
    Else
        path = Config.bmpPath & GrhData(GrhData(currentGrh).Frames(CurrentFrame)).FileNum & ".png"
    End If
    End If
...
grhList.Clear
DoEvents
Open App.path & "\export.txt" For Input As handle2
    Do While Not EOF(1)
        Line Input #1, Linea

        Index = InStr(1, Linea, "=", vbTextCompare)
        
        If Index > 0 Then
        
        i = mid(Linea, 4, Index - 4)
        
        Linea = Right(Linea, Len(Linea) - Index)
        
        If i > MaxGrh Then
            MaxGrh = i
            ReDim Preserve GrhData(i)
        End If
        
        With GrhData(i)
            .NumFrames = ReadField(1, Linea, 45)
            
            ReDim .Frames(1 To .NumFrames)
            If .NumFrames = 1 Then
                .FileNum = ReadField(2, Linea, 45)
                .sX = ReadField(3, Linea, 45)
                .sY = ReadField(4, Linea, 45)
                .pixelWidth = ReadField(5, Linea, 45)
                .pixelHeight = ReadField(6, Linea, 45)
            ...
            Else
                ...
                For Index = 1 To .NumFrames
                    .Frames(Index) = ReadField(1 + Index, Linea, 45)
                Next Index
                .Speed = ReadField(2 + .NumFrames, Linea, 45)
            End If
        End With
...
Exit Sub
Open App.path & "\export.ind" For Binary Access Write As handle
...
Private Sub mmGuardarTXT_Click()
Dim buffer As String
Dim i As Long
Dim frame As Integer
PB.max = UBound(GrhData())
PB.Visible = True
    Dim handle As Integer
    handle = FreeFile
    Open App.path & "\export.txt" For Binary Access Write As handle
For i = 1 To UBound(GrhData())
        If GrhData(i).NumFrames > 0 Then
        buffer = ""
            With GrhData(i)
                If .NumFrames > 1 Then
                    ...
                    buffer = buffer & "Grh" & i & "=" & .NumFrames
                    
                    For frame = 1 To .NumFrames
                        buffer = buffer & "-" & .Frames(frame)
                    Next frame
                    
                    buffer = buffer & "-" & .Speed & vbCrLf
                Else
                    buffer = buffer & "Grh" & i & "=1-" & .FileNum & "-" & .sX & "-" & .sY & "-" & .pixelWidth & "-" & .pixelHeight & vbCrLf
                End If
            End With
             Put handle, , buffer
        End If
...
Call SavePicture(previewer.image, App.path & "\" & currentGrh & ".png")
End Sub
...
If bmp > 0 Then
            Dim path As String
            If Right$(Config.bmpPath, 1) <> "\" Then
                path = Config.bmpPath & "\" & bmp & ".png"
            Else
                path = Config.bmpPath & bmp & ".png"
            End If
            
            currentPic.Picture = LoadPictureGDI(path)
End If
DibujarIt
End Sub
Sub DibujarIt()
...
Private Sub RedrawPicture(ByVal Grh As Long, ByVal frame As Long)
    ...
    On Error Resume Next
    'Clear picturebox
       'previewer.Picture = LoadPictureGDI("")
