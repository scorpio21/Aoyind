...
'*************************************************
    ...
    ElseIf Modo = eModoComercio.Venta Then
        ...
        ElseIf ObjData(Objeto.ObjIndex).Newbie = 1 Then
            ...
            Call WriteTradeOK(UserIndex)
            Exit Sub
        ElseIf Not NpcVendeItem(NpcIndex, Objeto.ObjIndex) And ((Npclist(NpcIndex).TipoItems <> ObjData(Objeto.ObjIndex).OBJType And Npclist(NpcIndex).TipoItems <> eOBJType.otCualquiera) Or Objeto.ObjIndex = iORO) Then
            Call WriteConsoleMsg(UserIndex, "Lo siento, no estoy interesado en este tipo de objetos.", FontTypeNames.FONTTYPE_INFO)
            Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
            Call WriteTradeOK(UserIndex)
            Exit Sub
        ElseIf ObjData(Objeto.ObjIndex).Caos = 1 Or ObjData(Objeto.ObjIndex).Real = 1 Then
            Call WriteConsoleMsg(UserIndex, "¡No puedes vender tu armadura faccionaria!.", FontTypeNames.FONTTYPE_INFO)
            Call EnviarNpcInv(UserIndex, UserList(UserIndex).flags.TargetNPC)
            Call WriteTradeOK(UserIndex)
            Exit Sub
    ...
    'Por si compra muy rapido :S
    Call FlushBuffer(UserIndex)
End Sub
Public Function NpcVendeItem(ByVal NpcIndex As Integer, ByVal ObjIndex As Integer) As Boolean
Dim LoopC As Integer
For LoopC = 1 To MAX_INVENTORY_SLOTS
    If Npclist(NpcIndex).Invent.Object(LoopC).ObjIndex = ObjIndex Then
        NpcVendeItem = True
        Exit Function
    End If
Next LoopC
...
'*************************************************
    ...
    For Slot = 1 To MAX_INVENTORY_SLOTS
        If Npclist(NpcIndex).Invent.Object(Slot).ObjIndex > 0 Then
            ...
            Call WriteChangeNPCInventorySlot(UserIndex, Slot, thisObj, val, UserNoPuedeUsarItem(UserIndex, thisObj.ObjIndex))
        Else
            Dim DummyObj As Obj
