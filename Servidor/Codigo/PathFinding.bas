...
IsWalkable = MapData(Map).Tile(row, Col).Blocked = 0 And Not HayAgua(Map, row, Col)
With Npclist(NpcIndex)
If MapData(Map).Tile(row, Col).UserIndex <> 0 Then
     If MapData(Map).Tile(row, Col).UserIndex <> .PFINFO.TargetUser Then
        IsWalkable = False
        ...
        End If
    End If
ElseIf MapData(Map).Tile(row, Col).NpcIndex <> 0 Then
    If MapData(Map).Tile(row, Col).NpcIndex <> .PFINFO.TargetNPC Then IsWalkable = False
End If
End With
End Function
