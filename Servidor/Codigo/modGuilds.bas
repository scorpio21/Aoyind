...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    If UserList(UserIndex).Stats.ELV < 35 Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 90 Or UserList(UserIndex).Stats.GLD < OROFUNDARCLAN Or Not TieneObjetos(ManualDeClan, 1, UserIndex) Then
        refError = "Para fundar un clan debes ser al menos nivel 35, tener el Manual de Clan, 90 skills en liderazgo y pagar " & OROFUNDARCLAN & " monedas de oro."
        Exit Function
    End If
