...
If getInterval(TActual, UserList(UserIndex).Counters.TimerLanzarSpell) >= IntervaloUserPuedeCastear Then
    If getInterval(TActual, UserList(UserIndex).Counters.TimerGolpeMagia) >= IntervaloGolpeMagia Then
...
If getInterval(TActual, UserList(UserIndex).Counters.TimerPuedeAtacar) >= IntervaloUserPuedeAtacar Then
    If getInterval(TActual, UserList(UserIndex).Counters.TimerMagiaGolpe) >= IntervaloMagiaGolpe Then
        If Actualizar Then
